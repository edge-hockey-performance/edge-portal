...
type Plan = { productId: number; variantId: number; weeklySellingPlanId: number; weeklyCents: number; upfrontCents: number };
const PLANS = new Map<number, Plan>([
  [47941773230243, { productId: 9212478029987, variantId: 47941773230243, weeklySellingPlanId: 3369599139, weeklyCents: 1300, upfrontCents: 30000 }],
  [47941775458467, { productId: 9212478980259, variantId: 47941775458467, weeklySellingPlanId: 3387031715, weeklyCents: 1900, upfrontCents: 44000 }],
]);
const SUPPORTED = new Set([
  "orders/paid", "subscription_contracts/update", "subscription_billing_attempts/success",
  "subscription_billing_attempts/failure", "subscription_billing_attempts/challenged",
]);

type Json = Record<string, unknown>;
function response(status: number, body: Json) { return new Response(JSON.stringify(body), { status, headers: { "content-type": "application/json; charset=utf-8" } }); }
...
function numericId(value: unknown): number | null {
  ...
  if (typeof value === "string") { const parsed = Number(value.split("/").pop() || value); return Number.isSafeInteger(parsed) ? parsed : null; }
  return null;
}
function gid(kind: string, value: unknown): string | null {
  const prefix = `gid://shopify/${kind}/`;
  if (typeof value === "string" && value.startsWith("gid://")) return value.startsWith(prefix) && numericId(value) ? value : null;
  const id = numericId(value); return id ? `${prefix}${id}` : null;
}
...
async function digest(body: Uint8Array) { return Array.from(new Uint8Array(await crypto.subtle.digest("SHA-256", body))).map((byte) => byte.toString(16).padStart(2, "0")).join(""); }
function properties(line: Json): Record<string, string> { const result: Record<string, string> = {}; for (const item of Array.isArray(line.properties) ? line.properties : []) { const entry = record(item); const name = String(entry.name || "").trim(); if (name) result[name] = String(entry.value || "").trim(); } return result; }
function sellingPlan(line: Json): number | null { const allocation = record(line.selling_plan_allocation); return numericId(allocation.selling_plan_id) || numericId(record(allocation.selling_plan).id) || numericId(line.selling_plan_id); }
function contractGid(payload: Json): string | null { const contract = record(payload.subscription_contract); return gid("SubscriptionContract", payload.subscription_contract_gid ?? payload.subscription_contract_id ?? contract.admin_graphql_api_id ?? contract.id ?? payload.admin_graphql_api_id); }
function firstContractLine(payload: Json): Json { const candidates = [payload.lines, payload.subscription_lines, record(payload.subscription_contract).lines]; for (const candidate of candidates) if (Array.isArray(candidate) && candidate.length === 1) return record(candidate[0]); return {}; }
function planTriple(line: Json) {
  ...
  return {
    ...
    variantId,
    sellingPlanId: sellingPlan(line),
  };
}
async function sendPortalInvite(supabase: ReturnType<typeof createClient>, membership: Json | null) {
  ...
  if (!membershipId || !playerId || String(membership?.status ?? "") !== "active") return;
...
Deno.serve(async (req: Request) => {
  ...
  let payload: Json;
  try { payload = JSON.parse(new TextDecoder().decode(raw)); } catch { return response(400, { error: "invalid_json" }); }
  ...
  const { data: inbox, error: inboxError } = await supabase.rpc("record_shopify_webhook_event", {
    event_webhook_id: webhookId, event_topic: topic, event_shop_domain: shop,
    event_payload_sha256: await digest(raw), event_subscription_contract_gid: contract,
  });
  ...
  const finish = async (status: "processed" | "ignored" | "failed", message: string | null = null) => {
    await supabase.rpc("finish_shopify_webhook_event", { event_webhook_id: webhookId, event_status: status, event_error_message: message, event_subscription_contract_gid: contract });
  };
  if (!SUPPORTED.has(topic)) { await finish("ignored"); return response(200, { status: "topic_ignored" }); }

  try {
    if (topic === "orders/paid") {
      const lines = (Array.isArray(payload.line_items) ? payload.line_items : []).map(record);
      const membershipLines = lines.filter((line) => { const variant = numericId(line.variant_id); return variant !== null && PLANS.has(variant); });
      if (!membershipLines.length) { await finish("ignored"); return response(200, { status: "no_membership_lines" }); }
      ...
      const { data: existing } = await supabase.from("membership_subscriptions").select("id,player_id,status").eq("shopify_order_gid", orderGid).maybeSingle();
      if (existing) {
        await sendPortalInvite(supabase, { membership_id: existing.id, player_id: existing.player_id, status: existing.status });
        await finish("processed"); return response(200, { status: "existing_membership_invitation_reconciled" });
      }
      ...
      const configured = triple.variantId ? PLANS.get(triple.variantId) : undefined;
      const completeTriple = configured && triple.productId === configured.productId && triple.sellingPlanId === configured.weeklySellingPlanId;
      const { error } = await supabase.rpc("process_shopify_contract_update", {
        event_id: webhookId, contract_gid: contract,
        origin_order_gid: gid("Order", payload.admin_graphql_api_origin_order_id ?? payload.origin_order_id),
        revision_id: numericId(payload.revision_id), contract_status: String(payload.status || ""),
      ...
      const { error } = await supabase.rpc("process_shopify_billing_attempt", {
        event_id: eventKey, contract_gid: contract, renewal_order_gid: orderGid,
