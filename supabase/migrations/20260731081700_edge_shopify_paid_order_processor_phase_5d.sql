CREATE UNIQUE INDEX membership_subscriptions_initial_order_idx
...
BEGIN
  ...
  ELSE
    ...
    INTO candidate_count, selected_player
    FROM public.players AS p
  ...
  IF NULLIF(trim(coalesce(subscription_contract_gid, '')), '') IS NULL THEN
  ...
  IF existing_membership IS NOT NULL THEN
    ...
    SET player_id = coalesce(ms.player_id, selected_player),
        ...
        status = CASE
          WHEN ms.status IN ('cancelled', 'expired') THEN ms.status
  ...
  ELSE
    ...
    INSERT INTO public.membership_subscriptions (
      ...
      shopify_order_gid,
      shopify_subscription_contract_gid,
      shopify_product_id,
      shopify_variant_id,
